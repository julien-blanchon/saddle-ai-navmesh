...
pub struct NavmeshLabE2EPlugin;

impl Plugin for NavmeshLabE2EPlugin {
    fn build(&self, app: &mut App) {
        ...
        if let Some(name) = scenario_name {
            if let Some(mut scenario) = scenarios::scenario_by_name(&name) {
                ...
                error!(
                    "[saddle_ai_navmesh_lab:e2e] Unknown scenario '{name}'. Available: {:?}",
...
fn parse_e2e_args(args: &[String]) -> (Option<String>, bool) {
    ...
    if !handoff {
        handoff = std::env::var("E2E_HANDOFF")
